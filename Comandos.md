| node -v                                                     | Versión de Node js                 |
|-------------------------------------------------------------|------------------------------------|
| npm install -g @angular/cli                                 | Instalar última versión de Angular |
| Set-ExecutionPolicy RemoteSigned -Scope CurrentUser         | Habilitar permisos                 |
| cls                                                         | comando para limpiar consola       |
| ng version                                                  | verificar versión de Angular       |
| git init                                                    | iniciar repositorio                |
| git status                                                  | verificar estado de mis archivos   |
| git add .                                                   | agregamos archivos al git          |
| git commit -m "V1_Angular"                                  | sellamos los archivos para enviar  |
| git remote add origin https://github.com/xxxxx              | enlazar repo                       |
| git remote -v                                               | verificar repositorio remoto       |
| git push                                                    | para enviar al repo remoto         |
| ng new mi-primer-angular --standalone --routing --style=css | crear proyecto                     |
| ng generate component view/user.component/user.component    | crear componente tipo vista        |
| ng generate service services/user.service/user.service      | para crear un servicio             |
| ng generate interface models/user.model/user.model          | para generar un modelo             |